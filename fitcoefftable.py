...
def main(argv):
    ...
    if(useBestFinf):
        ...
        #finfcolumn=2
        lcolumn=0
        dgcolumnstart=finfcolumn
        dgcolumnend=finfcolumn
    else:
        #finfcolumn=4
        #lcolumn=1
        #if finf isn't used, then this should sum from lmax+1 to infinty using the fit from lmin to lmax for every dg order and output these totals to a file for every time.
        lcolumn=0
        dgcolumnstart=1
        dgcolumnend=8
    nummodes=31
    ...
    if(useBestFinf):
        ...
        datatable=np.loadtxt("genrawdata"+str(t0)+".csv")
    ...
    selfForceDGordersArr=np.zeros([dgcolumnend-dgcolumnstart+1])
    selfForceDGordersArr2=np.zeros([dgcolumnend-dgcolumnstart+1])
    #selfForceDGordersUnextrap=np.zeros([dgcolumnend-dgcolumnstart+1])
    #selfForceDGordersUnextrapTot=np.zeros([dgcolumnend-dgcolumnstart+1])
    ...
    for finfcolumn in range(dgcolumnstart,dgcolumnend+1):
        ...
        selfForceDGordersArr2[finfi]=selfForce2
