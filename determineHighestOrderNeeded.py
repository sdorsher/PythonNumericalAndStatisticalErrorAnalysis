...
datatablefinf=np.loadtxt('selfForceOverTime_fit2.csv')
summedfinfcolumn = 2 #column for finf with unextrapolated sum
tcolumn=0
t0=390
psirfinfsummed=0
tindex=0
...
for order in range(len(datatablefiniteorder[tindex,1:])):
    absoluteerr[order]=abs(psirfinfsummed-datatablefiniteorder[tindex,order+1])
...
plt.title("|Psir(DG order)-Psir(Finf)| exponentially converges, no l-mode fit")
#plt.semilogy('log')
